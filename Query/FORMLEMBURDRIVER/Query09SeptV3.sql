...
SELECT 
    ...
    CASE 
        ...
        ELSE '-'
    END AS LamaKerja,
    -- Lama lembur (hanya jika lebih dari ShiftOut)
    CASE 
        WHEN a.AbsDateOut IS NOT NULL AND a.ShiftOut IS NOT NULL
        THEN 
            CASE 
                WHEN a.AbsDateOut > DATEADD(MINUTE, 0, 
                        CAST(CONVERT(DATE, a.AbsDateIn) AS DATETIME) + CAST(a.ShiftOut AS DATETIME))
                THEN 
    ...
    ISNULL(a.AbsAtdInName, '-')  AS KeteranganKehadiran,
    ISNULL(a.AbsAtdOutName, '-') AS KeteranganPulang
FROM RosterData r
